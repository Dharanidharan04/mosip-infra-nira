...
function installing_admin() {
  ...
  helm -n $NS install admin-hotlist mosip/admin-hotlist --set image.repository=niraqa/hotlist-service --set image.tag=pre-production --version $CHART_VERSION
  ...
  helm -n $NS install admin-service mosip/admin-service --set image.repository=niraqa/admin-service --set image.tag=pre-production -f values.yaml --set istio.corsPolicy.allowOrigins\[0\].prefix=https://$ADMIN_HOST --wait --version $CHART_VERSION


  echo Installing admin-ui
  helm -n $NS install admin-ui mosip/admin-ui  --set image.repository=niraqa/admin-service --set image.tag=pre-production -f values.yaml --set admin.apiUrl=https://$API_HOST/v1/ --set istio.hosts\[0\]=$ADMIN_HOST --version $CHART_VERSION
