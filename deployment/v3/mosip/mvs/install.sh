...
function installing_mvs() {
  ...
  helm -n $NS install mvs-ui mosip-helm/mvs-ui --set image.repository=niraqa/manual-verification-system-ui --set image.tag=tf_nira_qa_new --set mvs.apiUrl=https://$mvs_HOST --set istio.hosts[0]=$mvs_HOST --version $CHART_VERSION

  echo Installing ms service. Will wait till service gets installed.
  helm -n $NS install mvs-service mosip-helm/mvs-service --set image.repository=niraqa/manual-verification-service -set image.tag=tf_nira_qa_new --version $CHART_VERSION --wait
